...
export function BottomTabHeader(props: BottomTabHeaderProps) {
  ...
  const userAvatar = userInfoQuery.data?.profile_image ? { uri: userInfoQuery.data.profile_image } : null

  return (
    <View className="pt-safe px-8">
      <View className="h-[72px] relative flex flex-row items-center justify-between">
        <Avatar src={userAvatar} isLoading={userInfoQuery.isLoading} className="w-[40px] h-[40px]" iconSize="lg" />
